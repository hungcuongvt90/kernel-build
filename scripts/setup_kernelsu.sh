...
if [ "$kernelsu_variant" == "Official" ]; then
  ...
  KSU_API_VERSION=$(curl -fsSL "https://raw.githubusercontent.com/ReSukiSU/ReSukiSU/main/kernel/Makefile" | grep -m1 "KSU_VERSION_API :=" | awk -F'= ' '{print $2}' | tr -d '[:space:]')
  if [[ -z "$KSU_API_VERSION" || "$(printf '%s\n' "$KSU_API_VERSION" "4.0.0" | sort -V | head -n1)" != "4.0.0" ]]; then
    KSU_API_VERSION="4.0.0"
  fi
  echo "KSU_API_VERSION=$KSU_API_VERSION" >> "$GITHUB_ENV"
  ...
  echo "Done applying KernelSU ReSukiSU..."
fi
